...
The calculation was formidable and done without the aid of computing machinery of any kind.
...
At this time there is not a package in R that implements the method of moments used by Pearson, so I used the code from Carlos Enrique Amendola Cerin the appendix od his valuable PhD thesis [Algebraic Statistics of Gaussian Mixtures](https://depositonce.tu-berlin.de/bitstream/11303/7284/4/amendola_ceron_carlos.pdf).
...
# EM Algorithm
#### Using BIC to find best the model (E equal variance or V variable variance) and the optimal number of components 
...
densV = densityMclust(crabs.df$ratioj,  modelNames = "V", verbose = F, plot = F)
...
densE = densityMclust(crabs.df$ratiob,  modelNames = "E", verbose = F, plot = F)
...
mixplot = ggplot(crabs.df,aes(x = ratio)) +
  ...
  #geom_density(aes(y = ..density.. * (1000 * 0.004)), col = 2)+
    geom_function(
    fun = function(x) {
      (probE[1] * (dnorm(x, mean = meanE[1], sd = varianceE[1])) + (probE[2]) * (dnorm(x, mean = meanE[2], sd = varianceE[2]))) * 1000 * 0.004
    },
    linetype = "dashed",
    lwd = .8,
    aes(colour="Sum of the two comp.") 
